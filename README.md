...
项目已打包好离线部署包 `offline-bundle.zip`（约 14 MB）：

1. 解压 `offline-bundle.zip` 到目标目录
2. 复制 `.env.example` 为 `.env`
3. 编辑 `.env` 填入你的 LLM API 配置
4. 运行：`node dist/index.js`
...
powershell Compress-Archive -Path dist,node_modules,package.json,package-lock.json,.env.example,README.md -DestinationPath offline-bundle.zip
```
